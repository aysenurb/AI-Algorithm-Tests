...
class NQueensProblem(SearchProblem):
        ...
        for i in range(0, len(state)):
            for j in range (i+1, len(state)):
                ...
                if abs(p2-p1) == abs(i-j):
                    diagonal += 1
        total = lines + diagonal
        return math.comb(self.n,2) - total

    def crossover(self, state1, state2):
        point = int(self.n/2)
        crossed_list =[]
        for i in range(0,point):
           crossed_list.append(state1[i])
        for i in range(point, len(state2)):
            crossed_list.append(state2[i])
        crossed = ''.join([str(elem) for elem in crossed_list])
        return crossed
    ...
    def mutate(self, state):
        ...
        random_value = random.randint(1, self.n)
        temp[random_position] = random_value
        state =''.join([str(elem) for elem in temp])
        return state
    ...
    def generate_random_state(self):
        ...
        for _ in range(self.n - 1):
            s = random.choice(range(1, self.n))
            my_list.append(s)
        list2str = ''.join([str(elem) for elem in my_list])
        return list2str
...
myViewer = BaseViewer()
problem = NQueensProblem("12345678",8)
timer = TicToc()
a = timer.tic()
result = genetic(problem,population_size=25,mutation_chance=0.1,viewer=myViewer)
